...
def duration_to_seconds(duration_str):
    try:
        if duration_str.startswith('PT'):
            # Improved regex to handle optional groups
            hms_pattern = r'PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?'
            match = re.fullmatch(hms_pattern, duration_str)
            
            if match:
                hours, minutes, seconds = match.groups()
                print(f"Matched groups - Hours: {hours}, Minutes: {minutes}, Seconds: {seconds}")
                ...
                print(f"Duration string format is incorrect: {duration_str}")
                return None
        else:
            print(f"Duration string does not start with 'PT': {duration_str}")
            return None
    except Exception as e:
...
if stream=="Home":
    ...
    if st.button("Collect and Store Data"):
        ...
        if mydb and mycursor:
                ...
                st.success("Channel details Collected")
            #else:
...
# Assuming you have imported required libraries and set up MySQL connection properly
            ...
            try:
                ...
                df5 = pd.DataFrame(q5, columns=["Video Name", "No of Like_Count"]).reset_index(drop=True)
                df5.index += 1
                st.dataframe(df5)
            except mysql.connector.Error as err:
            ...
            results_formatted = [(channel_name, avg_duration) for channel_name, avg_duration in results]

            # Create DataFrame with correct column names
            df = pd.DataFrame(results_formatted, columns=["Channel Name", "Average Duration"]).reset_index(drop=True)
            df.index += 1  # Optional: Reset index starting from 1

            # Sort DataFrame by Average Duration
            df = df.sort_values(by="Average Duration")
